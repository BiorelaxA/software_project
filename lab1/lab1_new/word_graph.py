...
class WordGraph:
    ...
    def calc_shortest_path(self, word1: str, word2: str) -> Tuple[List[Tuple[str, str]], int]:
        """计算最短路径"""
        word1, word2 = word1.lower(), word2.lower()
        if word1 not in self.nodes or word2 not in self.nodes:
            return [], 0
    ...
    def calc_pagerank(self, word_freq: Dict[str, int], d: float = 0.85, iterations: int = 10) -> Dict[str, float]:
        ...
        for node in self.nodes:
            # TF: 词频 / 总词数
            tf = word_freq.get(node, 0) / total_words if total_words > 0 else 0
            # IDF: 1 / (出度 + 入度 + 1)
    ...
    while True:
        ...
        if choice == '1':
            ...
            if not path:
                ...
                print(f"Shortest path: {' -> '.join([f'{u}->{v}' for u, v in path])}")
                print(f"Path length: {length}")
                
        elif choice == '5':
            pr = graph.calc_pagerank()
